...
import { Gift, Users, Trophy, Dices } from "lucide-react";
...
export function AdminLotteryPage() {
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-24">
            <AdminHeader
                title="投票抽獎中心"
                subtitle="從已完成投票的選民中隨機抽出幸運兒，以提升投票參與率。"
            />
                    ...
                    <div className="flex flex-col md:flex-row md:items-end gap-6">
                        ...
                        <div className="w-full md:w-32 space-y-3">
                            ...
                            <input
                                type="number"
                                min="1"
                                className="w-full rounded-xl bg-[var(--color-surface-container-high)] border-2 border-transparent focus:border-[var(--color-primary)]/30 text-[var(--color-on-surface)] px-6 py-4 focus:outline-none text-center font-black text-lg"
                                value={drawCount}
                        ...
                        <Button
                            ...
                            loading={isDrawing}
                            variant="filled"
                            className="h-16 px-10 rounded-xl font-black text-lg elevation-2 hover:elevation-3 bg-[var(--color-primary)] text-[var(--color-on-primary)]"
                            icon={<Dices className={cn("w-6 h-6", isDrawing && "animate-spin")} />}
                        >
                            {isDrawing ? "抽籤中..." : "抽出得獎者"}
                        </Button>
                    </div>
                ...
                {result && (
                    <Card className="p-12 rounded-3xl border border-[var(--color-primary)]/30 bg-[var(--color-primary-container)]/10 elevation-2 animate-slide-up text-center overflow-hidden relative">
                        ...
                        <p className="text-[var(--color-on-surface-variant)] font-medium mb-12 relative z-10">
                            已從 {result.totalParticipants} 位已投票選民中，抽出 {result.drawCount} 位幸運得主
                        </p>
