          ...
          <Card className="p-0 overflow-hidden border border-[var(--color-outline-variant)]/30 elevation-1 rounded-xl bg-[var(--color-surface-container-low)]">
            ...
            <div className="p-6 space-y-6">
              <div className="space-y-2">
                <label className="block text-sm font-bold text-[var(--color-on-surface-variant)] px-1">
                  選擇選舉活動
                </label>
              ...
              <div
                ...
                onClick={() =>
                  !isElectionLocked && fileInputRef.current?.click()
                }
              >
                <div
                  className={cn(
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="font-bold text-sm">
                      {status.type === "success" ? "操作成功" : "操作失敗"}
                    </p>
                    <p className="text-xs opacity-80 mt-0.5 break-words">
                      {status.type === "success"
                        ? `成功匯入 ${status.result?.votersImported} 筆，略過 ${status.result?.duplicatesSkipped} 筆。`
                        : status.message}
                    </p>
                  </div>
                </div>
          ...
          <Card className="p-0 overflow-hidden border border-[var(--color-outline-variant)]/30 elevation-1 rounded-xl bg-[var(--color-surface)] min-h-[600px] flex flex-col">
            <div className="p-6 border-b border-[var(--color-outline-variant)]/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-[var(--color-on-surface-variant)] opacity-60 font-medium">
                  目前的選舉人名單預覽
                </p>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="sticky top-0 z-10 bg-[var(--color-surface)] shadow-sm">
                    <tr className="bg-[var(--color-surface-container-high)]/80 backdrop-blur-md">
                      <th className="px-8 py-4 text-[10px] font-black tracking-widest text-[var(--color-on-surface-variant)] uppercase">
                        學號資訊
                      </th>
                      <th className="px-6 py-4 text-[10px] font-black tracking-widest text-[var(--color-on-surface-variant)] uppercase">
                        所屬班級
                      </th>
                      <th className="px-8 py-4 text-[10px] font-black tracking-widest text-[var(--color-on-surface-variant)] uppercase text-right">
                        登記狀態
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[var(--color-outline-variant)]/20">
                    {filteredVoters.map((voter) => (
                      <tr
                        key={voter.id}
                        className="group hover:bg-[var(--color-surface-container-low)] transition-all"
                      >
                        ...
                        <td className="px-8 py-4 text-right">
                          {voter.identityCommitment ? (
                            <div
                              className="inline-flex items-center gap-1.5 text-green-600 dark:text-green-400 font-bold text-xs"
                              title={voter.identityCommitment}
                            >
                              <Fingerprint className="w-3.5 h-3.5" />
                              已登記
                            </div>
                          ) : (
                            <span className="text-xs font-bold text-[var(--color-on-surface-variant)] opacity-40">
                              未登記
                            </span>
                          )}
                        </td>
